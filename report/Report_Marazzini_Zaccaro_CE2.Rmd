...
With this function we are telling R that we only want links that start (\^) with "<https://beppegrillo.it>" and that after this expression take any value, except "category" and "jpg". By viewing the output, we saw that there were some NAs and some links that were duplicate, so we proceed to remove them:
